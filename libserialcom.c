...
void read_bytes(struct sp_port *port, WheelSystemState *state) {
    char buf[sizeof(WheelSystemState)];
    sp_nonblocking_read(port, buf, syzeof(buf));
    state = (WheelSystemState *)buf;
